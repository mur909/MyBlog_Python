...
def global_variable(request):
    allcategory = Category.objects.all() # 查询所有分类
    remen = Article.objects.filter(tui__id=2)[:6] # 主页右侧热门推荐
    tags = Tag.objects.all() # 右侧所有标签实现
    return locals()

"""首页"""
def index(request):
    banner = Banner.objects.filter(is_active=True)[0:4] # 查询幻灯片
